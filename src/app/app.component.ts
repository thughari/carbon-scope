...
import {
  DashboardFilters,
  FiltersBarComponent,
} from './filters-bar/filters-bar.component';
...
import { ALL_COUNTRIES, Country } from '../models/country-data'; // <--- IMPORT THIS
...
export class AppComponent implements OnInit {
  ...
  countries: Country[] = ALL_COUNTRIES.sort((a, b) =>
  ...
  trendYears: number[] = [2015, 2018, 2020, 2022, 2024];
  trendSeries: { name: string; data: number[] }[] = [];

  headerTotal = 0;
  headerHighestSector = '';
  headerYearRange: [number, number] = [2015, 2024];
    ...
    this.emissions
      ...
      .subscribe({
        next: (res: SectorSummaryResponse) => {
          ...
          this.headerTotal = Object.values(res.sectorTotals).reduce(
            (a, b) => a + b,
            0
          );
          ...
          this.trendSeries = entries.slice(0, 5).map(([sector, value]) => ({
            name: sector,
            data: [value * 0.8, value * 0.85, value * 0.9, value * 0.95, value],
          }));
  ...
  private fetchGlobalContext(gas: string) {
    ...
    this.emissions
      ...
      .subscribe((res) => {
        this.globalEmissionsTotal = Object.values(res.sectorTotals).reduce(
          (a, b) => a + b,
          0
        );
      });
    ...
    this.emissions
      ...
      .subscribe({
        next: (res) => {
          ...
          const realSum = Object.values(res.subsectorTotals).reduce(
            (sum, val) => sum + val,
            0
          );
